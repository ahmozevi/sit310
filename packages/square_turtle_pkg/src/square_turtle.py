...
def move_turtle_square(): 
    ...
    while not rospy.is_shutdown():
        ...
        cmd_vel_msg.angular.z = 0.0 # Ensure no rotation while moving straight
        velocity_publisher.publish(cmd_vel_msg) # Publish!

        rate.sleep()

        # Create a Twist message for turning 90 degrees 
        cmd_vel_msg = Twist()
        cmd_vel_msg.linear.x = 0.0  # Ensure no moving forward while moving rotating
        cmd_vel_msg.angular.z = 1.57 # Rotate 90 degrees in radians
        velocity_publisher.publish(cmd_vel_msg) # Publish!
